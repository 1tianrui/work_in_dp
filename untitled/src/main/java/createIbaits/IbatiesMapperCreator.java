...
public class IbatiesMapperCreator {
    ...
    public IbatiesMapperCreator startBuild(String id){
       mapper.append(this.createHeader()+"\r\n");
       mapper.append(this.createResultMap()+"\r\n");
       return this;
    }
    ...
    public IbatiesMapperCreator buildSelect(String selectID,List<Integer> selectResultIndexes,WhereCondition condition,boolean pageAble){
         mapper.append(this.createSelect(selectID,selectResultIndexes,condition,pageAble));
         return this;
    }
    private  String createResultMap(){
        StringBuilder builder = new StringBuilder();
        builder.append("<resultMap> id=\""+classEntity.getClassName()+"\"  class=\""+classEntity.getClassName()+"\">\r\n");
    ...
    private String createWhere(WhereCondition condition){
        ...
        for(int i=0;i<compareIndexes.size();i++){
            ...
            builder.append(" "+tableColumns.get(index)+""+compartor.get(index)+"#"+classColumns.get(index)+"#");
    ...
    private String createSelect(String selectID,List<Integer> selectResultIndexes,WhereCondition condition,boolean pageAble){
        List<ColumnEntry> tableColumns = tableEntity.getColumn();
        List<ColumnEntry> classColumns = classEntity.getAttributeMap();

        StringBuilder selectBuilder = new StringBuilder();
        selectBuilder.append("<select id=\""+selectID+"\" parameterClass=\"map\" resultMap=\"" + classEntity.getClassName() + "\"\r\n");
        StringBuilder selectContent = new StringBuilder();
        for(int i = 0;i < selectResultIndexes.size() ;i++){
            selectContent.append(tableColumns.get(selectResultIndexes.get(i)).getName());
